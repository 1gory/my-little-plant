// Procedural SVG rendering of the plant based on growth percentage.
// (seedSVG removed — seeds in cards are now PNGs from icons/seeds/.)
...
export function plantSVG({
  ...
  const soilY = 192;
  // A PNG pot has a higher rim than the procedural trapezoid — raise the soil
  // and the whole plant by lift pixels so the flower grows from the soil at the rim.
  const lift = pot?.icon ? 10 : 0;
  const baseY = soilY - lift;
  const cx = 100;

  // Pot + soil. If the pot has a PNG icon — draw it as a layer (the plant goes
  // on top, as if growing out of it); otherwise — a procedural trapezoid.
  const parts = [];
  if (pot?.icon) {
    // Soft shadow under the pot (a separate PNG), then the pot itself on top of it.
  ...
  if (growth < 4 && !finished) {
    // Seed in the soil / just sprouted.
  ...
  // Leaves along the stem, in pairs on each side.
